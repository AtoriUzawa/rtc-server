...
// Module bundles the live HTTP handler and WebSocket handler.
type Module struct {
	Handler   *Handler
	WSHandler *WSHandler
}

// New creates a new live module with its manager, service, and handlers.
...
// RegisterHTTP registers HTTP routes for the live module.
func (m *Module) RegisterHTTP(r *gin.RouterGroup) {
	RegisterRouter(r, m.Handler)
}

// RegisterWS registers WebSocket routes for the live module.
